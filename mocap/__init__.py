...
class MocapConstraint(bpy.types.PropertyGroup):
    ...
    type: EnumProperty(name="Type of constraint",
                       items=[("point", "Maintain Position", "Bone is at a specific point"),
                              ("freeze", "Maintain Position at frame",
                               "Bone does not move from location specified in target frame"),
            ...
            if performer_obj.data and enduser_obj.data:
                if performer_obj.data.name in bpy.data.armatures and enduser_obj.data.name in bpy.data.armatures:
                    ...
                    for bone in perf.bones:
                        footCol.prop(data=bone, property='foot', text='', icon='OBJECT_DATAMODE')
                    ...
                    if performer_obj.animation_data and performer_obj.animation_data.action:
                        extraSettings.prop(data=performer_obj.animation_data.action, property='name',
                                           text='Action Name')
...
class OBJECT_OT_RetargetButton(bpy.types.Operator):
    ...
    def poll(cls, context):
        ...
        if performer_obj:
            return activeIsArmature and isinstance(performer_obj[0].data, bpy.types.Armature) and performer_obj[
                0].animation_data
        else:
            return False
...
class OBJECT_OT_PathEditing(bpy.types.Operator):
    ...
    def poll(cls, context):
        if context.active_object:
            selected_objs = [obj for obj in context.selected_objects if
                             obj != context.active_object and isinstance(obj.data, bpy.types.Curve)]
